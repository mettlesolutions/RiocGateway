...
public class DocSubmissionClient {
    ...
    public Submission SoapClientCall(Submission sub, String SplitStr, String parentId, ArrayList<Document> docArryList) throws PropertyException{
        ...
        HIHConfigurationParam hihval = new HIHConfigurationParam();
        hihval.setHIH_Description(HIH_Description);
        ...
        rcinfo.setRcName(sub.getIntendedRecepient().getIntendedRecepient());

        LoB lobinfo = new LoB();
        lobinfo.setContentType(sub.getPurposeOfSubmission().getContentType());
        lobinfo.setPurposeOfSubmission(sub.getPurposeOfSubmission().getPurposeOfSubmission());


        DocSubmissionData subdata = new DocSubmissionData();
        subdata.setAuthorNPI(sub.getAuthorNPI());
        if(sub.getEsmdCaseId() != null)
        subdata.setEsmdCaseId(sub.getEsmdCaseId());
        if(sub.getEsMDClaimID() != null)
        subdata.setEsMDClaimID(sub.getEsMDClaimID());

        subdata.setTitle(sub.getTitle());
        if(sub.getAuthorType() != null)
        subdata.setAuthorType(sub.getAuthorType().getId());

        subdata.setSplitStr(SplitStr);
        subdata.setIntendedRecepient(rcinfo);
        subdata.setPurposeOfSubmission(lobinfo);
        subdata.setHihconfigvals(hihval);
        ...
        while(documentIterator.hasNext()){
         ...
         //   System.out.println(fileStr);
            if(tempdt.getDocument_type() == DocumentType.PDF)
                dtlib.setFiletype(DOCUMENT_TYPE);
            else if(tempdt.getDocument_type() == DocumentType.XML)
                dtlib.setFiletype(DOCUMENT_TYPE_XML);


            dtlib.setAttachmentControlNumber(tempdt.getAttachmentControlNumber());
            if(tempdt.getLanguage() != null)
            dtlib.setLanguage(tempdt.getLanguage().getId());

            documentPayloads.add(dtlib);

        }
        subdata.setDocArryList(documentPayloads);
